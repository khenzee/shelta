...
export default async function MaintenancePage() {
  const [requestsResponse, propertiesResponse, tenantsResponse, employeesResponse, unitsResponse] = await Promise.all([
  ...
  const maintenanceRequests = requestsResponse.ok ? adaptMaintenanceRequests((await requestsResponse.json()).items || []) : [];
